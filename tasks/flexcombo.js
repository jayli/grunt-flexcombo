...
// 默认的 liveReload 端口号
var DEFAULT_LIVERELOADPORT = 35729;
...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('flexcombo', 'Start FlexCombo.', function () {
        ...
        var livereload = options.livereload;	// 是否需要自动刷新，配置为自动刷新端口号 or true
        var livereloadPort = ((livereload === true) ? DEFAULT_LIVERELOADPORT : livereload); //livereload工作端口号
                    ...
                    jayli
                        ...
                        .after(function (statCode, filePath, fileBuffer, encoding) {
                            ...
                            if (livereload) {
                                ...
                                var liveReloadScriptPath = "http://" + localIpAddr + ":"
                                    + ((livereload === true) ? DEFAULT_LIVERELOADPORT : livereload)
                                    + "/livereload.js?snipver=1";
                                decodedFileContent = decodedFileContent.replace('</body>', '<script src="' + liveReloadScriptPath + '"></script></body>');
                            }
        ...
        if(livereload && livereloadPort) {
            console.log('\nLiveReload running at ' + utils.blue('http://127.0.0.1:' + livereloadPort));
            console.log('  > 可通过 flexocombo 配置项 ' + utils.yellow('livereload') + ' 更改端口号');
        }
